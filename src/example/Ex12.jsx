...
// use~로 시작하는 모든 기능들은 이곳에서 선언 (React hook)
  console.log('1번 실행')


//   3. useEffect(()=>{},[])
//  - 화면을 세팅한 직후 실행 
//  - 복잡하고 무거운 값들을 이곳에서 불러와줄것
//  - backend에서 보내주는 값, DB 값, 외부에서 가져온 API 값 ... 

  useEffect(()=>{
    console.log('3번 실행 - 화면 렌더링 완료 직후')

    // useEffect에는 가지고 오는데 오래걸리는 값들을 호출한다. 
    // 왜? 사용자 입장에서는 화면이 아예 안뜨는 것보다 순차적으로 뜨는 것을 더 빠르다고 느낌 => 그렇기 때문에 빨리 띄울 수 있는건 먼저 띄우고, 그 다음에 무거운 작업을 하는 것 
    ...
    */
    ...
    axios   // axios 라는 라이브러리를 이용해서 비동기 통신을 하겠다.
    .get(url)   // get 방식으로 데이터를 가져오겠다.
    .then(res=> {console.log(res)})     // then : 가져오는데 성공했으면 함수 실행
...
// - []안에 작성한 값이 바뀔 때 마다, 함수가 실행
// - useEffect(()=>{},[num]) 
// - num의 값이 바뀔 때 마다 useEffect 함수가 실행된다. 
